...
public:
    ...
    uint8_t next_bit() {
        ...
        // Calculate feedback via parity of tapped bits
        int feedback = 0;
...
std::vector<uint8_t> generate_afs_q(int prn, int variant) {
    // Generate PRN-specific primary Weil code (includes expansion at index p)
    std::vector<uint8_t> primary = generate_weil_primary(prn);
    
    // Generate PRN-specific tertiary Weil code (1500 chips + 1 appended 0)
    std::vector<uint8_t> tertiary = generate_weil_tertiary(prn);
    
    // Get secondary code per variant (Table 10)
    // S0=[1,1,1,0], S1=[0,1,1,1], S2=[1,0,1,1], S3=[1,1,0,1]
    uint8_t sec_c0, sec_c1, sec_c2, sec_c3;
    switch (variant) {
        case 0:  // S0
            sec_c0 = 1; sec_c1 = 1; sec_c2 = 1; sec_c3 = 0;
            break;
        case 1:  // S1
            sec_c0 = 0; sec_c1 = 1; sec_c2 = 1; sec_c3 = 1;
            break;
        case 2:  // S2
            sec_c0 = 1; sec_c1 = 0; sec_c2 = 1; sec_c3 = 1;
            break;
        case 3:  // S3
            sec_c0 = 1; sec_c1 = 1; sec_c2 = 0; sec_c3 = 1;
            break;
        default:
            sec_c0 = 1; sec_c1 = 1; sec_c2 = 1; sec_c3 = 0; // Default to S0
            break;
    }
    
    // AFS-Q assembly: chip-by-chip XOR of primary and tertiary with secondary code cycling
    // Each group of 4 chips from tertiary is XORed with one secondary code bit and the primary
    std::vector<uint8_t> afs_q;
    afs_q.reserve(primary.size() + tertiary.size());
    
    // TODO: Implement proper chip-by-chip interleaving per spec Section 3
    // For now, concatenate to maintain working executable (placeholder per backlog)
